...
function OrderList() {
  ...
  const handleCancelOrder = async (e, id) => {
    ...
    console.log(id);
    await axios
      .put(api + `/order/update/${id}`, { orderTracking: 20 }, { headers })
      .then((response) => {
        ...
        <div className="row">
              ...
              <ul className="nav custom-tab">
                ...
                <li
                  onClick={() => {
                    setActiveTab(2);
                    setRecords(
                      orderList.filter((item) => item.orderTracking === 1)
                    );
                  }}
                  className="nav-item"
                >
                ...
                <li
                  onClick={() => {
                    ...
                    setRecords(
                      orderList.filter(
                        (item) =>
                          item.orderTracking === 3 || item.orderTracking === 4
                      )
                    );
                  }}
                  className="nav-item"
                >
                  <div className={`nav-link ${activeTab === 3 && "active"}`}>
                    Đang vận chuyển
                  </div>
                </li>
                <li
                  onClick={() => {
                    setActiveTab(4);
                    setRecords(
                      orderList.filter((item) => item.orderTracking === 5)
                    );
                  }}
                  className="nav-item d-none d-lg-block"
                >
                  <div className={`nav-link ${activeTab === 4 && "active"}`}>
                    Đang giao
                  </div>
                </li>
                <li
                  onClick={() => {
                    setActiveTab(5);
                    setRecords(
                      orderList.filter((item) => item.orderTracking === 6)
                    );
                  }}
                  className="nav-item mr-0 d-none d-lg-block"
                >
                  <div className={`nav-link ${activeTab === 5 && "active"}`}>
                    Hoàn thành
                  </div>
                </li>
              </ul>
                      ...
                      <div className="price-order col-md-2">
                        <ul className="list-inline list-inline-dotted mb-3 mb-lg-2">
                          ...
                          <li className="list-inline-item">
                            <a href="#">
                              <h5 className="title">
                                {item.priceSale.toLocaleString("vi-VN")}đ
                              </h5>
                            </a>
                          </li>
                        </ul>
                        ...
                        item.orderTracking === 1 && (
                          <>
                            <button
                              ...
                              className="btn btn-primary mt-2 mr-2 text-white"
                            >
                              Thanh toán
                            </button>
                            <button
                              type="button"
                              onClick={(e) => handleCancelOrder(e, item._id)}
          ...
          <div className="col-lg-12 mt-4">
            <div className="main-button button-page-order">
              <a onClick={handlePrevPage} href="#product">
                <i className="fa fa-angle-double-left"></i>
              </a>
              <a href="#product">{pageNum}</a>
              <a onClick={handleNextPage} href="#product">
